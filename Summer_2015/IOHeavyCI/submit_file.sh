	...
	for k in 1 3 6 10; do
		for j in 1024 2048 4096 8192 16384 32768 65536 131072; do
			...
			for i in `seq 3 $(($k+2))`; do
				/nfs-7/t2tas/reader -f /data$i/xrootd/trimmed_infile_$i -b $j -a "$k    $_CONDOR_SLOT" | tee -a /tmp/BobaksReadTestOutput &   
			done
			wait
		done
	done
else
	echo "--Ignored Slot--" | tee -a /tmp/BobaksIgnoredOutput
	echo `uname -n` : $_CONDOR_SLOT | tee -a /tmp/BobaksIgnoredOutput
	sleep 600
fi
